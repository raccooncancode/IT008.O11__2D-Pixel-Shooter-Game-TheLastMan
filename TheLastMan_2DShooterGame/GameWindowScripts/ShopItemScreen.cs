...
namespace TheLastMan_2DShooterGame.GameWindowScripts
{
    public class ShopItemScreen
    {
        ...
        private void SetShopItemModal()
        {
            ...
            ShopItemModal.BackgroundImage = AssetsLoader.Instance.UI_BG["PanelBG"];
            ShopItemModal.BackgroundImageLayout = ImageLayout.Stretch;
        }
        private void SetBackgroundModal()
        {
            BackgroundModal = CustomControls.Instance.BackgroundModal(ShopItemModal);
        }
        private void SetShopItemUI()
        {
            ...
            for (int i = 0; i < tb.Rows.Count; i++)
            {
                ...
                buyBTN.Click += (object sender, EventArgs e) =>
                {
                    ...
                    if (quantity.Value > 0)
                    {
                        if (GameManager.Instance.P.GetGold() - int.Parse(price.Text) >= 0)
                        {
                            ...
                            MessageBox.Show("Bạn đã mua thành công " + quantity.Value.ToString() + " " + item_Pic.Name);
                            InventoryScreen.Instance.UpdateUI();
                        }
                        else
                        {
                            MessageBox.Show("Số vàng còn lại không đủ để mua");
                        }
                    }
                    else
                    {
                        MessageBox.Show("Vui lòng chọn số lượng của vật phẩm muốn mua");
                    }
                };
                border.BackgroundImage = AssetsLoader.Instance.UI_Borders["tile002"];
                ...
                item_Pic.BackgroundImage = AssetsLoader.Instance.GO_Item[tb.Rows[i].ItemArray[1].ToString()];
