...
namespace TheLastMan_2DShooterGame
{
    public class PlayNewScreen
    {
        ...
        private void SetPlayNewModal()
        {
            ...
            PlayNewModal.BackgroundImage = AssetsLoader.Instance.UI_BG["PanelBG"];
            PlayNewModal.BackgroundImageLayout = ImageLayout.Stretch;
        }
        private void SetBackgroundModal()
        {
            BackgroundModal = CustomControls.Instance.BackgroundModal(PlayNewModal);
        }
        ...
        private void SetMapSelectUI()
        {
            ...
            var mapImages = AssetsLoader.Instance.UI_Maps;
        ...
        private void Map_ListView_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (isOpenning())
            {
                if (Map_ListView.SelectedItems.Count == 0)
                    return;
                if (isOpenning() && Map_Selected_Name == null)
                {
                    MessageBox.Show("Bạn đã chọn map " + Map_ListView.SelectedItems[0].Text);
                }
        ...
        {
            e.Graphics.DrawImage(AssetsLoader.Instance.GO_Player["FrontIdle"], new Point(0, 0)) ;
        }
